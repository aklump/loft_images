...
QUnit.begin(function () {
  ...
  QUnit.storage.inlineSvgControl =
...
//
QUnit.test('Assert aspect_ratio works as expected custom child.', function (
  assert
) {
  var $el = $('.grandpa')
    .addClass('grandpa--aspect-ratio-child')
    .find('.father');
  var control = { w: 400 };
...
QUnit.test(
  'Assert aspect_ratio works as expected using ratio float.',
  function (assert) {
    var $el = $('.grandpa')
      .addClass('grandpa--aspect-ratio-ratio')
      .find('.father');
    var control = { w: 400 };
...
QUnit.test('Assert aspect_ratio works as expected with defaults.', function (
  assert
) {
...
QUnit.test('Assert SASS smart_bg() theme--rightbottom10px works.', function (
  assert
) {
  var $icon = $('.group-background__span');

  $icon.addClass('theme--rightbottom10px');
  assert.notStrictEqual($icon.height(), 24);
  assert.notStrictEqual($icon.width(), 24);
  assert.strictEqual(
    $icon.css('backgroundImage'),
  ...
  assert.strictEqual($icon.css('backgroundRepeat'), 'no-repeat');
  assert.strictEqual($icon.css('backgroundPosition'), '100% bottom 10px');
  assert.strictEqual($icon.css('backgroundSize'), 'auto');
  assert.strictEqual($icon.css('display'), 'inline');
  assert.strictEqual($icon.css('margin'), '0px');
  assert.strictEqual($icon.css('paddingTop'), '0px');
  assert.strictEqual($icon.css('paddingLeft'), '0px');
  assert.strictEqual($icon.css('paddingRight'), '44px');
  assert.strictEqual($icon.css('paddingBottom'), '54px');
});
QUnit.test('Assert SASS smart_bg() theme--centertop10px works.', function (
  assert
) {
  var $icon = $('.group-background__span');

  $icon.addClass('theme--centertop10px');
  assert.notStrictEqual($icon.height(), 24);
  assert.notStrictEqual($icon.width(), 24);
  assert.strictEqual(
    $icon.css('backgroundImage'),
  ...
  assert.strictEqual($icon.css('backgroundRepeat'), 'no-repeat');
  assert.strictEqual($icon.css('backgroundPosition'), '50% top 10px');
  assert.strictEqual($icon.css('backgroundSize'), 'auto');
  assert.strictEqual($icon.css('display'), 'inline');
  assert.strictEqual($icon.css('margin'), '0px');
  assert.strictEqual($icon.css('paddingTop'), '54px');
...
QUnit.test('Assert SASS smart_bg() theme--right-center works.', function (
  assert
) {
  var $icon = $('.group-background__span');
...
QUnit.test(
  'Assert root element receives custom classes in non svg and non retina environments.',
  function (assert) {
    ...
    $(document).bind('loftImagesDomReady', function () {
      assert.ok(
        $('html').hasClass('no-hi-res'),
        'html received custom "no-retina" class by default in non-retina environment.'
      );
      assert.ok(
        $('html').hasClass('no-line-art'),
        'html received custom "no-svgasimg" class by default in non-svg environment.'
      );
      $(document).unbind('loftImagesDomReady');
      done();
    });
...
QUnit.test(
  'Assert root element receives custom classes in svg and retina environments.',
  function (assert) {
    LoftImages.prototype.isRetina = function () {
      return true;
    };
    ...
    $(document).bind('loftImagesDomReady', function () {
      assert.ok(
        $('html').hasClass('hi-res'),
        'html received custom "retina" class by default in retina environment.'
      );
      assert.ok(
        $('html').hasClass('line-art'),
        'html received custom "svgasimg" class by default in svg environment.'
      );
      $(document).unbind('loftImagesDomReady');
      done();
    });
...
QUnit.test(
  'Assert processDom with sub context catches sub group only.',
  function (assert) {
    var done = assert.async();
    $(document).bind('loftImagesDomReady', function () {
      assert.ok($('#test .retina-1').attr('src'));
...
QUnit.test('Assert processDom with default context catches all', function (
  assert
) {
  var done = assert.async();
  $(document).bind('loftImagesDomReady', function () {
...
QUnit.test('Class root custom selector', function (assert) {
  ...
  assert.ok(
    $(custom).hasClass('no-svgasimg'),
    'html received "no-svgasimg" class by default in non-svg environment.'
  );
});
...
QUnit.test('Class root selector default, svg', function (assert) {
  ...
  images.init().processDom();
  assert.ok(
    $('html').hasClass('svgasimg'),
    'html received "svg" class by default in svg environment.'
  );
});
...
QUnit.test('Class root selector: default, non-retina', function (assert) {
  ...
  images.init().processDom();
  assert.ok(
    $('html').hasClass('no-retina'),
    'html received "no-retina" class by default in non-retina environment.'
  );
});
...
QUnit.test('Class root selector default, retina', function (assert) {
  ...
  assert.ok(
    ...
    'html received "retina" class by default in retina environment.'
  );
});

QUnit.test('Assert process methods return LoftImages', function (assert) {
  assert.ok(
    images.processRetinaImage($('#test .retina-1')) instanceof LoftImages,
    'returns LoftImages.'
  );
  assert.ok(
    images.processSvgImage($('#test .svg-1')) instanceof LoftImages,
    'returns LoftImages.'
  );
});
...
QUnit.test('Assert processSvgImage() handles non-jQuery argument.', function (
  assert
) {
  LoftImages.prototype.isSvg = function () {
    return false;
  ...
  assert.strictEqual(
    ...
    'images/svg.png',
    'Non-svg version applied in non-svg environment.'
  );
});

QUnit.test('Assert processSvgImage() works with default settings.', function (
  assert
) {
  ...
  assert.strictEqual(
    $image.attr('src'),
    'images/svg.svg',
    'Retina version applied in svg environment.'
  );
});

QUnit.test(
  'Assert processRetinaImage() handles non-jQuery argument.',
  function (assert) {
    LoftImages.prototype.isRetina = function () {
      return false;
    };
    ...
    assert.strictEqual(
      image.getAttribute('src'),
      'images/bitmap.jpg',
      'Non-retina version applied in non-retina environment.'
    );
  }
);

QUnit.test(
  'Assert processRetinaImage() works with default settings.',
  function (assert) {
    var $image = $('#test .retina-1');
    ...
    assert.strictEqual(
      $image.attr('src'),
      'images/bitmap@2x.jpg',
      'Retina version applied in retina environment.'
    );
  }
);
